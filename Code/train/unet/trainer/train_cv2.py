            ...
            if np.less(current, self.best):
                ...
                if self.save_weights_only:
                  # First save the model weights in local machine (on google compute engine)
                    self.model.save_weights('unet_1024_best_weights.hdf5', overwrite=True)
                    # Now copy the weights file to google compute storage path dtored in self.filepath 
                    with file_io.FileIO('unet_1024_best_weights.hdf5', mode='r') as input_f:
                        with file_io.FileIO(self.filepath + '/unet_1024_best_weights.hdf5', mode='w+') as output_f:
                            output_f.write(input_f.read())
                else:
                  # First save the model in local machine (on google compute engine)
                    self.model.save('unet_1024_best_model.h5', overwrite=True)
                    # Now copy the file to google compute storage path dtored in self.filepath 
                    with file_io.FileIO('unet_1024_best_model.h5', mode='r') as input_f:
                        with file_io.FileIO(self.filepath + '/unet_1024_best_model.h5', mode='w+') as output_f:
                            output_f.write(input_f.read())
        else:
            if self.save_weights_only:
              # First save the model weights in local machine (on google compute engine)
                self.model.save_weights('unet_1024_weights.hdf5', overwrite=True)
                # Now copy the weights file to google compute storage path dtored in self.filepath 
                with file_io.FileIO('unet_1024_weights.hdf5', mode='r') as input_f:
                    with file_io.FileIO(self.filepath + '/unet_1024_weights.hdf5', mode='w+') as output_f:
                        output_f.write(input_f.read())
            else:
              # First save the model in local machine (on google compute engine)
                self.model.save('unet_1024_model.h5', overwrite=True)
                # Now copy the file to google compute storage path dtored in self.filepath 
                with file_io.FileIO('unet_1024_model.h5', mode='r') as input_f:
                    with file_io.FileIO(self.filepath + '/unet_1024_model.h5', mode='w+') as output_f:
                        output_f.write(input_f.read())


def train_unet(train_dir, job_dir):
  """
